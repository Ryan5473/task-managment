...
export default function TaskCard({ task, onClick, onDuplicate }: TaskCardProps) {
  ...
  return (
    <div
      className="mb-2 p-3 bg-card rounded-md shadow-sm border border-border hover:shadow-md transition-shadow cursor-pointer group"
      onClick={onClick}
    >
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        ...
        {task.customFields.map(
          (field) =>
            field.value && (
              <div key={field.id} className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded-md">
                {field.name}:{" "}
